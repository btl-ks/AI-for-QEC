...
- [ ] **P4.18 统一具名随机流派生**：7 条随机流当前以人工挑选的裸偏移量从主种子派生，其中 RBM 洗牌与 CD 采样同为 `+41`（实际碰撞），模型初始化与训练集生成同为 `+0`（仅因分属 torch/numpy 未产生相同序列）。改为经 `numpy.random.SeedSequence` 按注册流名派生，未注册流名 fail loudly；消除 `qec_generator.py` 中与 `SPLIT_SEED_OFFSETS` 重复的常量表。**破坏性**：生成、训练、评估三端随机流全部变化，已生成数据集与历史 run 结果不可用新代码重现，不提供兼容模式。需求与证据见 OpenSpec change `unify-random-stream-derivation`。
