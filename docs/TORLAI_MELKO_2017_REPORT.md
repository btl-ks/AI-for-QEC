...
- **L=6 需要更大的解码步数预算。** 以 1000 步为上限时，超时占主导，失败率明显高于论文；把上限提高到 10000 步、复用同一模型重新解码后，p ≤ 0.09 的结果与 MWPM 一致，除 p=0.05 略高外也与论文读数一致；p ≥ 0.11 时 RBM 明显差于 MWPM。
...
| 解码 | 单链，平衡约 10² 步后取首条兼容链，超时算失败 | 单链，预热 25 步；步数上限 L=4 为 400，L=6 为 1000 与 10000 |
...
**问题二：L=6 的解码步数上限不够。**
以 1000 步为上限时，L=6 的超时比例从 8%（p=0.05）升到 62%（p=0.15），超时都算作失败，因此失败率明显高于论文。成功解码样本所需步数的分布尾部很长（p=0.08 时中位数 78 步、99% 分位数 825 步），贴近上限。
...
以下问题不影响实验结论，但造成了失败 run 或重复计算。相关的失败 run 已在整理时删除。

| 问题 | 影响 | 处理 |
|---|---|---|
| CUDA 不支持整数矩阵乘法，冒烟解码报错 | 冒烟阶段 1 个 run 失败 | 兼容性检查改用 float32 计算（本次复现前已修复） |
| 笔记本睡眠 | L=4 一个点的解码暂停约 11 小时，恢复后原地继续，未重跑 | 插电时关闭睡眠与休眠 |
| 精确 MWPM 最多处理 18 个 defect | L=6 在 p ≥ 0.10 时对照阶段报错，4 个点重新训练 | 超限样本改用 PyMatching；在 L=6 上核对两者匹配权重完全相同，失败率相差不超过 0.008 |
| 数据集身份哈希包含整份配置 | 只改解码参数的重解码找不到原数据集，11 个点启动即失败（无计算、无数据） | 按源 run 的配置校验并引用其数据集 |
| benchmark 阶段改写 evaluate 已登记哈希的 `metrics.json` | evaluate 阶段记录的哈希全部失效（数据未被篡改） | 已列入计划 P0.19，尚未修复 |

为便于以后复用，本次新增了两项功能：Notebook 的 `REUSE_CHECKPOINT_FROM_RUN` 开关（跳过训练、复用已有模型重新解码，并逐条核对结果），以及训练中一个结果不变的提速改动（参数、每步损失与随机数状态均与原实现逐位一致）。训练和解码的断点续跑尚未实现，已列入计划 P0.14–P0.19 与 P3.5。
