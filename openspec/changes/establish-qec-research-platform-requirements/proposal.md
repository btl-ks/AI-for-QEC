...
项目已有研究主题、架构、技术栈和优化计划，但需求分散在多份文档中，缺少统一、可验证、可由实现任务持续追踪的行为规格。现在需要把研究平台从“论文复现脚本集合”收敛为一组明确的能力契约，使训练数据生成、解码研究、容灾恢复、性能部署和 AI 自动验证能够按同一套验收标准演进。
...
| --- | --- | --- |
| `implement-experiment-lifecycle-recovery` | 实验身份、run 状态、中断、checkpoint 与恢复 | 可用故障注入证明 interrupted、resume、不可变产物与恢复一致性 |
...
| `build-neural-training-workbench` | PyTorch trainer、模型 registry、checkpoint、神经与混合 decoder、数据高效训练 | 配置真实生效、训练可恢复，并在冻结协议下完成公平对照 |
...
- 建立实验生命周期契约，统一配置解析、不可变数据与 run、断点恢复、产物身份、失败语义和论文导出。
...
- `research/experiment-lifecycle`: 配置、dataset、run、checkpoint、恢复、产物和论文导出的可追溯生命周期。
