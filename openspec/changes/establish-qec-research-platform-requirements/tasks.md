...
- [ ] 2.2 完成 P0.19 阶段产物不可变和结束时哈希复核，验证后续阶段改写已登记文件会使 run 失败
- [ ] 2.3 运行正式 runner smoke、损坏产物和身份不匹配集成测试，验证所有路径产生明确终态且零步骤或部分执行不为 success
...
- [ ] 8.4 运行完整 smoke、数据完整性、统一 LER、样本效率和性能报告验收，并验证所有未实施后期能力仍显式失败
