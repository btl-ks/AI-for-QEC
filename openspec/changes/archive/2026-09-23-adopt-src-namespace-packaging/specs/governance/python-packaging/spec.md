...
建立可验证的 Python 分发边界，使开发检出与构建产物通过一致的 `ai_qec` 导入名称暴露公共 Notebook API，同时避免仓库根目录偶然遮蔽已安装包或依赖包初始化副作用。

## ADDED Requirements

### Requirement: 可安装源码使用 src 布局
项目 SHALL 将 `ai_qec` 的可安装 Python 源码置于 `src` 源码根目录，并 SHALL 通过项目构建元数据从该目录发现分发包；仓库根目录 MUST NOT 同时保留第二份 `ai_qec` 源码树。

#### Scenario: 从源码构建分发包
- **WHEN** 维护者使用声明的构建后端从干净检出构建 wheel
- **THEN** wheel SHALL 包含 `src/ai_qec` 下声明的模块，并且 SHALL NOT 依赖仓库根目录存在可导入的 `ai_qec` 目录

### Requirement: 包目录采用隐式 namespace
项目 SHALL 使用隐式 namespace package 发现所有 `src/ai_qec` 子包，并且 `src/ai_qec` 与测试树中 MUST NOT 存在 `__init__.py`；模块发现 MUST NOT 依赖包初始化文件或其副作用。
...
#### Scenario: 检查源码树
- **WHEN** 维护者扫描 `src/ai_qec` 与 `tests` 目录
